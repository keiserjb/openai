...
use BackdropOpenAI\OpenAI\Responses\Realtime\Session\InputAudioTranscription;
...
final class SessionResponse implements ResponseContract
{
    ...
    {
        $tools = array_map(fn(array $tool): FunctionTool => match ($tool['type']) {
            'function' => FunctionTool::from($tool),
        }, $attributes['tools']);
        return new self(clientSecret: ClientSecret::from($attributes['client_secret']), inputAudioFormat: $attributes['input_audio_format'], inputAudioTranscription: isset($attributes['input_audio_transcription']) ? InputAudioTranscription::from($attributes['input_audio_transcription']) : null, instructions: $attributes['instructions'], maxResponseOutputTokens: $attributes['max_response_output_tokens'], modalities: $attributes['modalities'], outputAudioFormat: $attributes['output_audio_format'], temperature: $attributes['temperature'], toolChoice: $attributes['tool_choice'], tools: $tools, turnDetection: isset($attributes['turn_detection']) ? TurnDetection::from($attributes['turn_detection']) : null, voice: $attributes['voice']);
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return ['client_secret' => $this->clientSecret->toArray(), 'input_audio_format' => $this->inputAudioFormat, 'input_audio_transcription' => $this->inputAudioTranscription?->toArray(), 'instructions' => $this->instructions, 'max_response_output_tokens' => $this->maxResponseOutputTokens, 'modalities' => $this->modalities, 'output_audio_format' => $this->outputAudioFormat, 'temperature' => $this->temperature, 'tool_choice' => $this->toolChoice, 'tools' => array_map(static fn(FunctionTool $tool): array => $tool->toArray(), $this->tools), 'turn_detection' => $this->turnDetection?->toArray(), 'voice' => $this->voice];
    }
}
