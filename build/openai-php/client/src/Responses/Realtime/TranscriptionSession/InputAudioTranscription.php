...
namespace BackdropOpenAI\OpenAI\Responses\Realtime\TranscriptionSession;
...
final class InputAudioTranscription implements ResponseContract
{
     ...
     */
    private function __construct(public readonly string $language, public readonly string $model, public readonly string $prompt)
    {
    }
    /**
    ...
    {
        return new self(language: $attributes['language'], model: $attributes['model'], prompt: $attributes['prompt']);
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return ['language' => $this->language, 'model' => $this->model, 'prompt' => $this->prompt];
    }
}
