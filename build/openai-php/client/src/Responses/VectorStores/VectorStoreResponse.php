...
final class VectorStoreResponse implements ResponseContract, ResponseHasMetaInformationContract
{
    ...
    {
        return ['id' => $this->id, 'object' => $this->object, 'name' => $this->name, 'status' => $this->status, 'usage_bytes' => $this->usageBytes, 'created_at' => $this->createdAt, 'file_counts' => $this->fileCounts->toArray(), 'metadata' => $this->metadata, 'expires_after' => $this->expiresAfter?->toArray(), 'expires_at' => $this->expiresAt, 'last_active_at' => $this->lastActiveAt];
    }
}
