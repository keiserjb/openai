...
namespace BackdropOpenAI\OpenAI\Responses\Assistants;
...
final class AssistantResponse implements ResponseContract, ResponseHasMetaInformationContract
{
     ...
     */
    private function __construct(public string $id, public string $object, public int $createdAt, public ?string $name, public ?string $reasoningEffort, public ?string $description, public string $model, public ?string $instructions, public array $tools, public ?AssistantResponseToolResources $toolResources, public array $metadata, public ?float $temperature, public ?float $topP, public string|AssistantResponseResponseFormat $responseFormat, private readonly MetaInformation $meta)
    {
    }
    /**
    ...
    {
        $tools = array_map(fn(array $tool): AssistantResponseToolCodeInterpreter|AssistantResponseToolFileSearch|AssistantResponseToolFunction => match ($tool['type']) {
        ...
        $responseFormat = is_array($attributes['response_format']) ? AssistantResponseResponseFormat::from($attributes['response_format']) : $attributes['response_format'];
        return new self($attributes['id'], $attributes['object'], $attributes['created_at'], $attributes['name'], $attributes['reasoning_effort'] ?? null, $attributes['description'], $attributes['model'], $attributes['instructions'], $tools, isset($attributes['tool_resources']) ? AssistantResponseToolResources::from($attributes['tool_resources']) : null, $attributes['metadata'], $attributes['temperature'], $attributes['top_p'], $responseFormat, $meta);
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        $response = ['id' => $this->id, 'object' => $this->object, 'created_at' => $this->createdAt, 'name' => $this->name, 'reasoning_effort' => $this->reasoningEffort, 'description' => $this->description, 'model' => $this->model, 'instructions' => $this->instructions, 'tools' => array_map(fn(AssistantResponseToolCodeInterpreter|AssistantResponseToolFileSearch|AssistantResponseToolFunction $tool): array => $tool->toArray(), $this->tools), 'tool_resources' => $this->toolResources?->toArray(), 'metadata' => $this->metadata, 'temperature' => $this->temperature, 'top_p' => $this->topP, 'response_format' => is_string($this->responseFormat) ? $this->responseFormat : $this->responseFormat->toArray()];
