...
final class ThreadMessageResponseContentImageFile implements ResponseContract
{
    ...
    use ArrayAccessible;
    use Fakeable;
    private function __construct(public string $fileId, public ?string $detail)
    {
    }
    /**
    ...
    {
        return new self($attributes['file_id'], $attributes['detail'] ?? null);
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return array_filter(['file_id' => $this->fileId, 'detail' => $this->detail], fn(?string $value): bool => $value !== null);
    }
}
