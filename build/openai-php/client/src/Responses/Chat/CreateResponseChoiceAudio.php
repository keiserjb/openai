...
final class CreateResponseChoiceAudio implements ResponseContract
{
    ...
    use ArrayAccessible;
    use Fakeable;
    private function __construct(public readonly string $id, public readonly string $data, public readonly int $expiresAt, public readonly string $transcript)
    {
    }
    /**
    ...
    {
        return new self(id: $attributes['id'], data: $attributes['data'], expiresAt: $attributes['expires_at'], transcript: $attributes['transcript']);
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return ['id' => $this->id, 'data' => $this->data, 'expires_at' => $this->expiresAt, 'transcript' => $this->transcript];
    }
}
