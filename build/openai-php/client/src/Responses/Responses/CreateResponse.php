...
use BackdropOpenAI\OpenAI\Actions\Responses\OutputObjects;
use BackdropOpenAI\OpenAI\Actions\Responses\OutputText;
use BackdropOpenAI\OpenAI\Actions\Responses\ToolChoiceObjects;
use BackdropOpenAI\OpenAI\Actions\Responses\ToolObjects;
...
use BackdropOpenAI\OpenAI\Responses\Responses\ToolChoice\FunctionToolChoice;
use BackdropOpenAI\OpenAI\Responses\Responses\ToolChoice\HostedToolChoice;
...
final class CreateResponse implements ResponseContract, ResponseHasMetaInformationContract
{
    ...
    public static function from(array $attributes, MetaInformation $meta): self
    {
        ...
        return new self(id: $attributes['id'], background: $attributes['background'] ?? null, object: $attributes['object'], createdAt: $attributes['created_at'], status: $attributes['status'], error: isset($attributes['error']) ? GenericResponseError::from($attributes['error']) : null, incompleteDetails: isset($attributes['incomplete_details']) ? CreateResponseIncompleteDetails::from($attributes['incomplete_details']) : null, instructions: $attributes['instructions'] ?? null, maxToolCalls: $attributes['max_tool_calls'] ?? null, maxOutputTokens: $attributes['max_output_tokens'], model: $attributes['model'], output: $output, outputText: OutputText::parse($output), parallelToolCalls: $attributes['parallel_tool_calls'], previousResponseId: $attributes['previous_response_id'], prompt: isset($attributes['prompt']) ? ReferencePromptObject::from($attributes['prompt']) : null, promptCacheKey: $attributes['prompt_cache_key'] ?? null, safetyIdentifier: $attributes['safety_identifier'] ?? null, serviceTier: $attributes['service_tier'] ?? null, reasoning: isset($attributes['reasoning']) ? CreateResponseReasoning::from($attributes['reasoning']) : null, store: $attributes['store'] ?? \true, temperature: $attributes['temperature'], text: isset($attributes['text']) ? CreateResponseFormat::from($attributes['text']) : null, toolChoice: $toolChoice, tools: $tools, topLogProbs: $attributes['top_logprobs'] ?? null, topP: $attributes['top_p'], truncation: $attributes['truncation'] ?? null, usage: isset($attributes['usage']) ? CreateResponseUsage::from($attributes['usage']) : null, user: $attributes['user'] ?? null, verbosity: $attributes['verbosity'] ?? null, metadata: $attributes['metadata'] ?? [], meta: $meta);
    }
    /**
     * {@inheritDoc}
     */
