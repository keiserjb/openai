...
final class CustomToolCallOutput implements ResponseContract
{
     ...
     */
    private function __construct(public readonly string $callId, public readonly string $output, public readonly string $type, public readonly string $id)
    {
    }
    /**
    ...
    {
        return new self(callId: $attributes['call_id'], output: $attributes['output'], type: $attributes['type'], id: $attributes['id']);
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return ['type' => $this->type, 'call_id' => $this->callId, 'id' => $this->id, 'output' => $this->output];
    }
}
