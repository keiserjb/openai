...
final class InputMessageContentInputFile implements ResponseContract
{
     ...
     */
    private function __construct(public readonly string $type, public readonly string $fileData, public readonly string $fileId, public readonly string $filename)
    {
    }
    /**
    ...
    {
        return new self(type: $attributes['type'], fileData: $attributes['file_data'], fileId: $attributes['file_id'], filename: $attributes['filename']);
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return ['type' => $this->type, 'file_data' => $this->fileData, 'file_id' => $this->fileId, 'filename' => $this->filename];
    }
}
