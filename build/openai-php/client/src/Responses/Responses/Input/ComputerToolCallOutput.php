...
final class ComputerToolCallOutput implements ResponseContract
{
     ...
     */
    private function __construct(public readonly string $callId, public readonly string $id, public readonly ComputerToolCallOutputScreenshot $output, public readonly string $type, public readonly array $acknowledgedSafetyChecks, public readonly string $status)
    {
    }
    /**
    ...
    {
        $acknowledgedSafetyChecks = array_map(fn(array $acknowledgedSafetyCheck) => AcknowledgedSafetyCheck::from($acknowledgedSafetyCheck), $attributes['acknowledged_safety_checks']);
        return new self(callId: $attributes['call_id'], id: $attributes['id'], output: ComputerToolCallOutputScreenshot::from($attributes['output']), type: $attributes['type'], acknowledgedSafetyChecks: $acknowledgedSafetyChecks, status: $attributes['status']);
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return ['call_id' => $this->callId, 'id' => $this->id, 'output' => $this->output->toArray(), 'type' => $this->type, 'acknowledged_safety_checks' => array_map(fn(AcknowledgedSafetyCheck $acknowledgedSafetyCheck) => $acknowledgedSafetyCheck->toArray(), $this->acknowledgedSafetyChecks), 'status' => $this->status];
    }
}
