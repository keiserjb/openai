...
final class OutputImageGenerationToolCall implements ResponseContract
{
     ...
     */
    private function __construct(public readonly string $id, public readonly ?string $result, public readonly string $status, public readonly string $type)
    {
    }
    /**
    ...
    {
        return new self(id: $attributes['id'], result: $attributes['result'] ?? null, status: $attributes['status'], type: $attributes['type']);
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return ['id' => $this->id, 'result' => $this->result, 'status' => $this->status, 'type' => $this->type];
    }
}
