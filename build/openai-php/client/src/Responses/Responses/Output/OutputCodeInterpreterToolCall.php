...
use BackdropOpenAI\OpenAI\Responses\Responses\Output\CodeInterpreter\CodeFileOutput;
use BackdropOpenAI\OpenAI\Responses\Responses\Output\CodeInterpreter\CodeTextOutput;
...
final class OutputCodeInterpreterToolCall implements ResponseContract
{
     ...
     */
    private function __construct(public readonly string $code, public readonly string $id, public readonly ?array $outputs, public readonly string $status, public readonly string $type, public readonly string $containerId)
    {
    }
    /**
    ...
    public static function from(array $attributes): self
    {
        ...
        if (is_array($attributes['outputs'])) {
            $outputs = array_map(static fn(array $output): CodeFileOutput|CodeTextOutput => match ($output['type']) {
                'files' => CodeFileOutput::from($output),
                'logs' => CodeTextOutput::from($output),
            }, $attributes['outputs']);
        }
        return new self(code: $attributes['code'], id: $attributes['id'], outputs: $outputs, status: $attributes['status'], type: $attributes['type'], containerId: $attributes['container_id']);
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return ['code' => $this->code, 'id' => $this->id, 'outputs' => $this->outputs ? array_map(static fn(CodeFileOutput|CodeTextOutput $output): array => $output->toArray(), $this->outputs) : null, 'status' => $this->status, 'type' => $this->type, 'container_id' => $this->containerId];
    }
}
