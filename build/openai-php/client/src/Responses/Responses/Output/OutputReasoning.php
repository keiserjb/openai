...
final class OutputReasoning implements ResponseContract
{
     ...
     */
    private function __construct(public readonly string $id, public readonly array $summary, public readonly string $type, public readonly ?string $encryptedContent, public readonly ?string $status)
    {
    }
    /**
    ...
    {
        $summary = array_map(static fn(array $summary): OutputReasoningSummary => OutputReasoningSummary::from($summary), $attributes['summary']);
        return new self(id: $attributes['id'], summary: $summary, type: $attributes['type'], encryptedContent: $attributes['encrypted_content'] ?? null, status: $attributes['status'] ?? null);
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return ['id' => $this->id, 'summary' => array_map(static fn(OutputReasoningSummary $summary): array => $summary->toArray(), $this->summary), 'type' => $this->type, 'encrypted_content' => $this->encryptedContent, 'status' => $this->status];
    }
}
