...
final class OutputLocalShellCallAction implements ResponseContract
{
     ...
     * @param  'exec'  $type
     */
    private function __construct(public readonly array $command, public readonly array $env, public readonly string $type, public readonly ?int $timeoutMs, public readonly ?string $user, public readonly ?string $workingDirectory)
    {
    }
    /**
    ...
    {
        return new self(command: $attributes['command'], env: $attributes['env'], type: $attributes['type'], timeoutMs: $attributes['timeout_ms'] ?? null, user: $attributes['user'] ?? null, workingDirectory: $attributes['working_directory'] ?? null);
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return ['command' => $this->command, 'env' => $this->env, 'type' => $this->type, 'timeout_ms' => $this->timeoutMs, 'user' => $this->user, 'working_directory' => $this->workingDirectory];
    }
}
