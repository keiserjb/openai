...
use BackdropOpenAI\OpenAI\Responses\Responses\Output\OutputMessageContentOutputTextAnnotationsContainerFile as AnnotationContainerFile;
use BackdropOpenAI\OpenAI\Responses\Responses\Output\OutputMessageContentOutputTextAnnotationsFileCitation as AnnotationFileCitation;
use BackdropOpenAI\OpenAI\Responses\Responses\Output\OutputMessageContentOutputTextAnnotationsFilePath as AnnotationFilePath;
use BackdropOpenAI\OpenAI\Responses\Responses\Output\OutputMessageContentOutputTextAnnotationsUrlCitation as AnnotationUrlCitation;
...
final class OutputMessageContentOutputText implements ResponseContract
{
     ...
     */
    private function __construct(public readonly array $annotations, public readonly string $text, public readonly string $type)
    {
    }
    /**
    ...
    {
        $annotations = array_map(fn(array $annotation): AnnotationFileCitation|AnnotationFilePath|AnnotationUrlCitation|AnnotationContainerFile => match ($annotation['type']) {
        ...
        return new self(annotations: $annotations, text: $attributes['text'], type: $attributes['type']);
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return ['annotations' => array_map(fn(AnnotationContainerFile|AnnotationFileCitation|AnnotationFilePath|AnnotationUrlCitation $annotation): array => $annotation->toArray(), $this->annotations), 'text' => $this->text, 'type' => $this->type];
    }
}
