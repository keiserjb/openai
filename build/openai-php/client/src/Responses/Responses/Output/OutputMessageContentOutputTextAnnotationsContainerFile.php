...
final class OutputMessageContentOutputTextAnnotationsContainerFile implements ResponseContract
{
     ...
     */
    private function __construct(public readonly string $fileId, public readonly string $type, public readonly ?string $text, public readonly ?int $startIndex, public readonly ?int $endIndex)
    {
    }
    /**
    ...
    {
        return new self(fileId: $attributes['file_id'], type: $attributes['type'], text: $attributes['text'] ?? null, startIndex: $attributes['start_index'] ?? null, endIndex: $attributes['end_index'] ?? null);
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        $result = ['file_id' => $this->fileId, 'type' => $this->type];
        if ($this->text !== null) {
