...
namespace BackdropOpenAI\GuzzleHttp\Handler;

use BackdropOpenAI\GuzzleHttp\Exception\ConnectException;
use BackdropOpenAI\GuzzleHttp\Exception\RequestException;
use BackdropOpenAI\GuzzleHttp\Promise as P;
use BackdropOpenAI\GuzzleHttp\Promise\FulfilledPromise;
use BackdropOpenAI\GuzzleHttp\Promise\PromiseInterface;
use BackdropOpenAI\GuzzleHttp\Psr7\LazyOpenStream;
use BackdropOpenAI\GuzzleHttp\TransferStats;
use BackdropOpenAI\GuzzleHttp\Utils;
use BackdropOpenAI\Psr\Http\Message\RequestInterface;
use BackdropOpenAI\Psr\Http\Message\UriInterface;
/**
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx): PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \BackdropOpenAI\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri): string
    {
        ...
        $redactedUriString = \BackdropOpenAI\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf): void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \BackdropOpenAI\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \BackdropOpenAI\GuzzleHttp\Psr7\Utils::streamFor($sink);
