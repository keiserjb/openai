(function ($, Backdrop, CKEditor5) {
  ...
  class OpenAI extends CKEditor5.core.Plugin {
    ...
    // Add updateStatus as a class method
    updateStatus(statusText, editorContainer) {
    ...
    init() {
      ...
      editor.ui.componentFactory.add('OpenAI', locale => {
        ...
        buttonView.set({
          ...
          withText: true,
          tooltip: false,
          icon: '<svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="2138.000000pt" height="2136.000000pt" viewBox="0 0 2138.000000 2136.000000" preserveAspectRatio="xMidYMid meet"> <g transform="translate(0.000000,2136.000000) scale(0.100000,-0.100000)" fill="#000000" stroke="none"> <path d="M9405 20200 c-196 -7 -294 -17 -445 -46 -41 -8 -118 -21 -170 -29\n' +
            ...
            '116 69 195 101 50 21 93 17 120 -11z"/></g></svg>',
        });
        ...
        // Define the dropdown items with custom styles and commands/links
        const items = [
          { label: 'Text Completion', commandName: 'openaiCompleteText' },
          { label: 'Adjust tone/voice', commandName: 'openaiAdjustTone' },
          { label: 'Summarize', commandName: 'openaiSummarize' },
          { label: 'Translate', commandName: 'openaiTranslate' },
          { label: 'Reformat/correct HTML', commandName: 'openaiReformat'},
          { label: 'Help & Support', href: 'https://github.com/backdrop-contrib/openai/issues', isLink: true },
        ];

        const dropdownItems = items.map(item => {
          const dropdownItemView = new CKEditor5.ui.ButtonView(locale);
          dropdownItemView.set({
            label: item.label,
            withText: true,
          });

          // Style each dropdown item
          dropdownItemView.extendTemplate({
            attributes: {
              class: 'openai-dropdown-item',
              style: {
                display: 'block',
                padding: '5px 10px',
                'text-align': 'left',
          ...
          // Define command or link actions for each item
          if (item.isLink) {
            dropdownItemView.on('execute', () => {
              window.open(item.href, '_blank');
            });
          } else {
            dropdownItemView.on('execute', () => {
              this.handleCommand(item.commandName);
            });
          }
        ...
        // Add dropdown items to the dropdown view
        dropdownView.panelView.children.addMany(dropdownItems);
    ...
    loadOpenAISettings(format) {
      ...
      console.log('OpenAI settings:', Backdrop.settings.filter?.formats?.[format]?.editorSettings?.openai);

      return Backdrop.settings.filter?.formats?.[format]?.editorSettings?.openai || {};
    }

    handleCommand(commandName) {
      switch (commandName) {
        case 'openaiCompleteText':
          this._showTextCompletionForm();
          break;
        case 'openaiAdjustTone':
          this._showAdjustToneForm();
          break;
        case 'openaiSummarize':
          this._summarizeSelectedText();
          break;
        case 'openaiTranslate':
          this._showTranslateForm();
          break;
        case 'openaiReformat':
  ...
  _closeForm(formElement) {
    ...
    _createForm({ labelText, inputId, submitCallback, cancelCallback }) {
      // Check if a form already exists and remove it
      const existingForm = document.getElementById('ck-openai-form');
      if (existingForm) {
      ...
      formElement.id = 'ck-openai-form'; // Assign a unique ID
    ...
    `;
      ...
      formElement.addEventListener('keydown', trapFocus);

      // Bind submit and cancel events
      ...
      formElement.querySelector('.ck-button-cancel').addEventListener('click', (event) => {
        event.preventDefault();
        cancelCallback(formElement);
        formElement.removeEventListener('keydown', trapFocus); // Remove event listener when form is closed
    ...
    // Ensure the form is properly removed when closed
    _closeForm(formElement) {
      if (formElement && formElement.parentNode) {
        formElement.parentNode.removeChild(formElement);
      }
    ...
    _showTextCompletionForm() {
      ...
      this.updateStatus('formReady', editorContainer);

      this._createForm({
        labelText: 'Ask ChatGPT for an idea or suggestion:',
        inputId: 'promptInput',
        submitCallback: (formElement) => {
          const inputValue = formElement.querySelector('#promptInput').value.trim();
          if (inputValue) {
        ...
        cancelCallback: (formElement) => {
          this.updateStatus('cancelled', editorContainer);
          this._closeForm(formElement);
        }
      });
    }
    ...
    _showAdjustToneForm() {
      ...
      this._createForm({
        labelText: 'Enter an adjective / descriptive phrase:',
        inputId: 'toneInput',
        submitCallback: (formElement) => {
    ...
    _summarizeSelectedText() {
      const editorContainer = this.editor.ui.view.editable.element.parentElement;
      this.updateStatus('Preparing to send text to OpenAI...', editorContainer);
    ...
    _showTranslateForm() {
      ...
      this._createForm({
        labelText: 'Translate selection into (enter a language):',
...
_writeHTML(html, range) {
  ...
  _sendPromptToOpenAI(prompt, editorContainer) {
    ...
    if (!currentSettings || Object.keys(currentSettings).length === 0) {
      ...
      this.updateStatus('Error: OpenAI settings are missing', editorContainer);
      return;
    }

    this.updateStatus('Request sent to OpenAI, awaiting response...', editorContainer);
    ...
    fetch('/api/openai-ckeditor/completion', {
      ...
      .then(response => {
        if (!response.ok) {
          this.updateStatus('Error: Server response not OK', editorContainer);
          throw new Error(`Server error: ${response.statusText}`);
        }
        this.updateStatus('Response received, processing...', editorContainer);
